...
{
    public class EnvShooter : Env, MonoMethods
    {
        int width = 20;
        int height = 20;
        ...
        {
            public bool right, up, down, left, charge;
            public float aim;
        }
        ...
        public EnvShooter(Type[] botTypes)
        {
        ...
        void MonoMethods.LoadContent(SpriteBatch spriteBatch, ContentManager content, GraphicsDeviceManager graphics)
        {
            ...
            for (int i = 0; i < this.bots.Length; i++)
            {
                ...
                } while (map[x, y] || this.bots.Any(f => f.pos == pos));
            ...
            for (int i = 0; i < bots.Length; i++)
            {
                if (bots[i].Alive)
                {
                    ...
                    if (move != Vector2.Zero)
                    {
                        ...
                        if (bots[i].charge > 0)
                            move *= 0.5f;
                        bots[i].Move(move * Bot.SPEED);
                    }

                    bots[i].aim = actions[i].aim;

                    if (actions[i].charge)
                    {
                        if (bots[i].ammo > 0)
                        {
                            bots[i].charge += Bot.CHARGESPEED;
                            if (bots[i].charge > 1)
                                bots[i].charge = 1;
                        }
                    }
                    else if (bots[i].charge > 0)
                    {
                        //shoot
                        Vector2 velocity = new Vector2((float)Math.Cos(bots[i].aim), (float)Math.Sin(bots[i].aim)) * bots[i].charge * Bot.MAXSHOOTSPEED;
                        bullets.Add(new Bullet(bots[i].pos, velocity, bots[i].Id, bots[i].GetInternalColor(), this));
